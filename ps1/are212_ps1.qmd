...
# Paths ==================
path <- "C:/Users/d2012/Documents/ARE 212/ARE212_2026\ps1"
...
```
```{r}
wdi_per_capita_log <- read_csv(file = file.path(path_data, "are212_gold_ps1_wdi_clean.csv"))
```
...
from question 17 because we are centering the data at zero and keeping the slope the same. 
...
### Verifying FWT another way. 
Regress y on residual from reg of 1 and GDPpc^2 on GDP
```{r}
ols_1_gdp <- 
  ols(
    x = wdi_per_capita_quad %>% select(GDPpc_thou),
